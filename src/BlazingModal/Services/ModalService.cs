...
public class ModalService : IModalService
{
    ...
    public IModalReference Show<TComponent>(ModalOptions options) where TComponent : IComponent
        => Show<TComponent>(string.Empty, new ModalParameters(), options);
    ...
    public IModalReference Show<TComponent>(ModalParameters parameters) where TComponent : IComponent
        => Show<TComponent>(string.Empty, parameters, new ModalOptions());
    ...
    public IModalReference Show<TComponent>(ModalParameters parameters, ModalOptions options) where TComponent : IComponent
        => Show<TComponent>(string.Empty, parameters, options);
    ...
    public IModalReference Show(Type contentComponent, string title, ModalParameters parameters, ModalOptions options)
    {
        ...
        RenderFragment modalContent = builder =>
        {
            ...
            foreach ((string name, object? value) in parameters.Parameters)
            {
        ...
        void ModalInstance(RenderTreeBuilder builder)
        {
            builder.OpenComponent<BlazingModalInstance>(0);
            builder.SetKey($"BlazingModalInst_{modalInstanceId}");
            ...
            builder.AddComponentReferenceCapture(5, compRef => modalReference!.ModalInstanceRef = (BlazingModalInstance)compRef);
            builder.CloseComponent();
        }
    }
